...
Make a plot showing the average estimate of μ_cap on the y axis and the true value of μ on the x axis. 
```{r}
Ave_True_plot = 
  sim_df %>% 
  group_by(mu) %>% 
...
Make a second plot the average estimate of μ_cap only in samples for which the null was rejected on the y axis and the true value of μ on the x axis. 
```{r}
Ave_In_Reject_Plot = 
  sim_df %>% 
  filter(p.value < 0.05) %>% 
...
* The sample average of μ_cap across tests for which the null is rejected is approximately equal to the true value of μ
* 